...
namespace SkiaSharpControl
{
	public class SRectangle
	{
		...
		//构造
		public SRectangle(SKPoint skPoint1, float width, float height, SKPaint sKPaint, Action<Cursor> ChangeCursor, SKGLControl sKGLControl)
		{
		...
		public void Draw(SKCanvas canvas)
		{
			...
			canvas.DrawRect(this.skPoint1.X,this.skPoint1.Y, this.width,this.height, this.sKPaint);
		}

		//sKGLControl MouseMove事件处理方案
		public void HandleMouseCoordinates(object sender, MouseEventArgs e)
		{
			isHover = SkiaTool.IsPointInsideRectangle(skPoint1, width, height, e.Location);
			if (isHover)
			{
