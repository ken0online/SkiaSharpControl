...
namespace SkiaSharpControl
{
	public class SLine
	{
		...
		private SKPoint skPoint1, skPoint2;
		//内部变量
		...
		//构造
		public SLine(SKPoint skPoint1,SKPoint skPoint2, SKPaint sKPaint, Action<Cursor> ChangeCursor, SKGLControl sKGLControl)
		{
			this.sKPaint = sKPaint;
			this.skPoint1 = skPoint1;
			this.skPoint2 = skPoint2;
		...
		{
			canvas.DrawLine(this.skPoint1, this.skPoint2, this.sKPaint);
		}

		//sKGLControl MouseMove事件处理方案
		public void HandleMouseCoordinates(object sender, MouseEventArgs e)
		{
			isHover = SkiaTool.IsMouseNear(skPoint1,skPoint2,e.Location,5);
			if (isHover)
			{
			...
			if (isMouseDown)
			{
				...
				skPoint2.X += xIncrement;
				skPoint2.Y += yIncrement;
			}
			oldPoint = e.Location;
		}
